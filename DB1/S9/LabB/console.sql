--CREATE SCHEMA tiendalab2505;

--SET search_path TO tiendalab2505;
...
$$
    BEGIN
        INSERT INTO clientes(clienteid, cedula_ruc, nombrecia, nombrecontacto, direccioncli)VALUES (p_id, p_ruc, p_nombre,p_contacto,p_direccion);
    END;
$$ LANGUAGE plpgsql;

-- READ (GET)

CREATE OR REPLACE FUNCTION SP_CLIENTE_SEL(
    p_id int
    )
    RETURNS TABLE(clienteid INTEGER, cedula_ruc varchar, nombrecia varchar, nombrecontacto varchar, direccioncli varchar) AS
$$
    BEGIN
        RETURN QUERY
        SELECT clienteid, cedula_ruc, nombrecia, nombrecontacto, direccioncli
        FROM clientes
...
CREATE OR REPLACE FUNCTION SP_CLIENTE_UPD(
    p_id int,
    p_ruc varchar,
    p_nombre varchar,
    p_contacto varchar,
...
$$
    BEGIN
        UPDATE clientes SET cedula_ruc = p_ruc,
                            nombrecia = p_nombre,
                            nombrecontacto = p_contacto,
                            direccioncli = p_direccion
        WHERE clienteid = p_id;
    END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION SP_CLIENTE_DEL(
    p_id int
    )
    RETURNS void AS
$$
    BEGIN
        DELETE FROM clientes WHERE clienteid = p_id;
    END;
$$ LANGUAGE plpgsql;


--SELECT SP_CLIENTE_INS(1001, ’1044182707’, ’Teofilo Chambilla’, ’Teofilo’,’’);
SELECT * FROM get_cliente_by_ruc('1678999891')
