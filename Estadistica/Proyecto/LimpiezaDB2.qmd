...
Continuando con la limpieza, queremos que la columna `type` corresponda a lenguajes de programación `pl`:

```{r}
dataframe %>% select(type) %>% group_by(type) %>% summarize(n())
```

Se eliminan todos los registros que no sean `pl` (programming languages) o `queryLanguage` (Lenguaje de consulta)
